...
const AveragePopularityLinePlot = ({ data }) => {
  // Função para calcular a média de popularidade por artista e ano
  const calculateAveragePopularity = (data) => {
    const avgPopularity = [];
  ...
  const plotData = Object.keys(data).map((artist) => {
    const artistData = avgPopularityData.filter(
      (d) => d.artist === capitalize(artist.replace("_", " "))
    );
    return {
