...
const LineChart: React.FC<LineChartProps> = ({ data }) => {
  ...
  const aggregateDataByAlbumSize = (artistData: any[]) => {
  ...
  const calculateTrendline = (x: number[], y: number[]) => {
    const n = x.length
    ...
    return [
      {
        ...
        mode: 'markers',
        marker: { color },
        hovertemplate: `<b>Artist:</b> ${capitalize(artist.replace('_', ' '))}<br><b>Album Size:</b> %{x}<br><b>${capitalize(metric)}:</b> %{y}<extra></extra>`,
      },
      {
